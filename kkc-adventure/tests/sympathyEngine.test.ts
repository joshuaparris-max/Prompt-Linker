...
describe('calculateHeatCost', () => {
  test('high similarity produces lower cost than low similarity', () => {
    const highCost = calculateHeatCost(0.9, 60);
    const lowCost = calculateHeatCost(0.1, 60);
    expect(highCost).toBeLessThan(lowCost);
  });

  test('weak alar produces higher cost than strong alar', () => {
    const weakCost = calculateHeatCost(0.5, 30);
    const strongCost = calculateHeatCost(0.5, 70);
...
describe('resolveOutcome (deterministic)', () => {
  test('same state always produces the same outcome', () => {
    ...
    const first = resolveOutcome(30, state);
    const second = resolveOutcome(30, state);
  ...
  test('a state with risk=5 produces success', () => {
    ...
    // 47 >= 5, so success
    expect(resolveOutcome(5, state)).toBe('success');
  });
});
...
describe('adjudicate', () => {
  test('source not in inventory returns blocked with no_source_material', () => {
    ...
    const result = adjudicate({ source_item_id: 'iron_drab', target_item_id: 'candle_tallow', intent: 'test' }, state);
  ...
  test('valid materials and healthy state returns success', () => {
    ...
    const result = adjudicate({ source_item_id: 'candle_tallow', target_item_id: 'candle_tallow', intent: 'test' }, state);
...
describe('applySympathyResult', () => {
  test('after success, warmth is lower', () => {
    ...
    if (result.outcome !== 'blocked') {
      const newState = applySympathyResult(state, result);
  ...
  test('after backlash, injuries array is non-empty', () => {
    ...
    const state = baseState();
    const newState = applySympathyResult(state, backlashResult);
...
describe('buyMaterial', () => {
  test('buying candle_tallow at fishery deducts correct drabs', () => {
    ...
    const { newState, message } = buyMaterial('candle_tallow', 2, state);
  ...
  test('buying at wrong location returns failure message', () => {
    ...
    const { newState, message } = buyMaterial('candle_tallow', 1, state);
